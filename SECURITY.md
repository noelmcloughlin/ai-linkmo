...
Restating that design here would imply this repository controls it. It does not. A `TRUST_LADDER_SKILLS_REF` bump changes only the skill the scheduled run installs; the workflows and the scripts under `.lokf/scripts/` move only when copied again from `skills/ktl-sidecar/templates/`, from the release the pin names or from its `main` when a fix should not wait for the next tag. `knowledge-preflight.sh` reports drift on its `copies` line where a sidecar is installed to compare against. What this repository does own, and is accountable for:
