...
Restating that design here would imply this repository controls it. It does not. A `TRUST_LADDER_SKILLS_REF` bump changes only the skill the scheduled run installs; the workflows and the files under `.lokf/scripts/` and `.lokf/m365/` move only when copied again from `skills/ktl-sidecar/templates/`, from the release the pin names or from its `main` when a fix should not wait for the next tag. `knowledge-preflight.sh` reports drift on its `copies` line where a sidecar is installed to compare against. What this repository does own, and is accountable for:

- **The deployed copy.** Both files carry every job and check the template does. The wrapper is byte-identical to its template, and the workflow differs from its template only in its comments and in the pin the install step names. Anything further that is edited locally becomes this repository's responsibility rather than the template's; `diff` against `skills/ktl-sidecar/templates/` in a checkout of `knowledge-trust-ladder` to confirm.
- **The switches.** The agent step runs only while the `KNOWLEDGE_LIBRARIAN_ENABLED` repository variable is `true`; `AGENT_CLI` chooses *which* non-interactive agent runs, never *what command*, and its credential, the `AGENT_API_KEY` secret or the job token, reaches only the agent; the workflow triggers only on `schedule` and `workflow_dispatch`. All three are readable in this repository's own YAML.
