...
- how the two-job split keeps the agent away from any write-scoped token, and why the `publish` job's own checks - path confinement derived from the patch itself, refusal of any added `by: human:` claim - are the backstop rather than the wrapper's: [Prompt-injection guards](https://github.com/noelmcloughlin/lokf-agent-skills/blob/main/docs/threat-model.md#prompt-injection-guards);
...
- why a skill's stated scope is prose, not a permission, when you run it interactively: [Interactive use: scope is advisory, not enforced](https://github.com/noelmcloughlin/lokf-agent-skills/blob/main/docs/threat-model.md#interactive-use-scope-is-advisory-not-enforced);
- why the review PR it opens does not trigger this repository's `pull_request` checks, and what stands in for them: [Repository hardening](https://github.com/noelmcloughlin/lokf-agent-skills/blob/main/docs/threat-model.md#repository-hardening).
