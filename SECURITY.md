...
`.github/workflows/knowledge-librarian.yaml` and `.lokf/scripts/knowledge-librarian.sh` are copies of the `lokf-sidecar` template in [`knowledge-trust-ladder`](https://github.com/noelmcloughlin/knowledge-trust-ladder), and their security design is that repository's, documented once there:
