...
from lib.api.constants import EXCLUDED_PARAMS
...
try:
    ...
    # Not installed as a distribution (typical for `uv run` from source).
    # Fall back to the canonical version recorded in pyproject.toml.
    try:
...
# Hosts that are always permitted for AI_ATLAS_API_URL without an explicit
# opt-in. Anything else requires AI_ATLAS_API_URL_ALLOW_REMOTE=1 to guard
# against accidentally pointing the CLI at an untrusted server (which would
# otherwise receive every query verbatim).
...
def _resolve_default_api_base_url() -> str:
    ...
    Accepts ``AI_ATLAS_API_URL`` only when:
      * the scheme is http or https,
      * the host is in the local allowlist, OR
      * ``AI_ATLAS_API_URL_ALLOW_REMOTE`` is set to a truthy value.

    Otherwise emits a warning and falls back to ``http://localhost:8000``.
    """
...
# Default request timeout (seconds) for HTTP calls; overridable via --timeout.
...
def _configure_logging(verbose: bool = False) -> None:
    ...
    Called from :func:`main` rather than at import time so importing the CLI
    module (e.g. from tests) doesn't mutate global logging state. We only
    quiet down the AIAtlasNexus loggers that have been observed to be noisy
    - other third-party loggers are left alone.
    """
...
def schema_to_argparse_config(param_name: str, param_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if schema_type == 'integer':
        ...
        # All boolean flags use store_true semantics (including --pretty,
        # which is auto-detected when omitted but can still be explicitly set).
        config['action'] = 'store_true'
    elif schema_type == 'number':
        config['type'] = float
    elif schema_type == 'array':
        # ``--foo a b c`` -> ['a', 'b', 'c']. The inner type is best-effort:
        # default to ``str`` because OpenAPI ``items`` may not be present.
        items = schema.get('items', {}) or {}
        item_type = items.get('type', 'string')
        type_map = {'integer': int, 'number': float, 'boolean': str, 'string': str}
        config['type'] = type_map.get(item_type, str)
        config['nargs'] = '*'
...
def create_argument_parser(
    ...
    all_parameters: Optional[Dict[str, Dict[str, Any]]] = None,
    ...
    Args:
        ...
        all_parameters: Optional pre-extracted parameter map. If omitted, will
            be extracted from ``openapi_spec`` (extraction is non-trivial so
            callers that already have it should pass it in to avoid duplicate
            work).
...
Use API mode (default) for fast queries, or local mode for offline operation.""",
    ...
    # Extract parameters dynamically from OpenAPI spec (use caller-provided
    # map if available so the spec isn't walked twice per invocation).
...
def parse_arguments() -> argparse.Namespace:
    ...
    all_parameters, _, required_parameters = extract_parameters_from_spec(openapi_spec)

    # Create and parse arguments with standard argparse
    parser = create_argument_parser(valid_scopes, 'api', openapi_spec, all_parameters=all_parameters)
...
def call_api_server(endpoint: str, args_dict: Dict[str, Any], base_url: str = DEFAULT_API_BASE_URL) -> None:
    ...
    timeout = float(args_dict.get('timeout') or DEFAULT_REQUEST_TIMEOUT)
    ...
    try:
        ...
        if endpoint == ARES_ENDPOINT:
            response = requests.post(
                url, json=filtered_args, timeout=timeout, verify=True
            )
        else:
            response = requests.get(
                url, params=filtered_args, timeout=timeout, verify=True
            )
        ...
        # Force the next detect_server_url call to actually probe so we don't
        # keep hitting the same dead host until the TTL expires.
        invalidate_server_url_cache()
...
def call_local_handler(endpoint: str, args_dict: Dict[str, Any]) -> None:
    ...
    # Don't mutate the caller's dict; we strip internal keys aggressively below.
...
def main() -> None:
    ...
    # Configure baseline logging here (was previously at module import time);
    # keeps logging mutation out of ``import lib.cli.cli``.
