<script lang="ts">
  ...
  const activeFilterItems = $derived([
    // standard fields
    ...DROPDOWN_FIELDS.filter((field) => field.param !== 'schemaField').map((field) => {
      const backendKey = field.param;
...
{#snippet clearButtonIcon()}
  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24">
    ...
    <path stroke="#555" stroke-width="2" stroke-linecap="round" d="M9 9l6 6m0-6l-6 6" />
  </svg>
{/snippet}
...
>
  ...
  <div
    ...
    class="mb-0 box-border flex w-full flex-col items-start gap-0 self-end rounded-2xl border-2 border-blue-400 bg-linear-to-br from-blue-200/90 via-blue-100/90 to-blue-50/80 px-2 py-1 pb-1.5 text-black shadow-sm"
  >
    <h3 itemprop="title" class="mb-0 w-full text-sm font-bold tracking-wide text-black">Search</h3>
    ...
    >
      ...
      <input
        ...
        itemprop="value"
        type="text"
        class="input input-sm w-full max-w-full rounded-lg bg-white pr-8 pl-3 font-normal text-black placeholder-black"
        style="height: 32px; min-height: 32px; font-size: 0.875rem;"
        placeholder="text to find"
        bind:value={filters.searchText}
        bind:this={searchInputRef}
        oninput={(e) => filters.setParam('searchText', (e.target as HTMLInputElement).value)}
        autocomplete="off"
      />
      {#if filters.searchText}
        <button
          ...
          class="absolute top-1/2 right-2 m-0 -translate-y-1/2 cursor-pointer border-0 bg-transparent p-0 text-gray-400 hover:text-gray-700 focus:outline-none"
          onclick={() => {
  ...
  <div
    ...
    itemscope
    itemtype="https://schema.org/WebPageElement"
    class="flex w-full flex-col items-start gap-0 rounded-2xl border-2 border-blue-400 bg-linear-to-br from-blue-200/90 via-blue-100/90 to-blue-50/80 px-2 py-1 text-black shadow-sm"
    style="width:100%;padding-bottom: 0.5rem;box-sizing:border-box;align-self:flex-end;"
  >
    <h3 itemprop="title" class="mb-0 text-sm font-bold tracking-wide text-black">Filtering</h3>
    <div
      itemprop="additionalProperty"
      itemscope
      itemtype="https://schema.org/PropertyValue"
      class="box-border flex w-full flex-col gap-0"
    >
      <div
      ...
      >
        ...
        <label
          for="param-id-input"
          itemprop="name"
          class="label label-text pl-1 text-sm leading-tight text-black"
      ...
      <input
        ...
        placeholder="id ..."
        class="input input-sm w-full max-w-full rounded-lg bg-white pl-3 font-normal text-black placeholder-black {filters.isRelatedMode
      ...
      {#each filterConfigs.filter( (config) => UI_WANTED_FILTERS[String(endpoint.current)]?.includes(config.key) ) as config (config.id)}
        {#if config.param !== 'hasRelatedRisk' && config.param !== 'hasRelatedAction'}
          ...
          >
            ...
            <div class="w-full">
              <SelectDropdown
                ...
                disabled={config.disabledExtra() ||
                  (!filters.isRelatedMode && filters.getOptions(config.param).length === 0)}
            ...
            <meta itemprop="value" content={String(filters.getParam(config.param) || '')} />
          </div>
        {/if}
      {/each}
    </div>
  ...
  {#each relatedParamConfigs as relatedConfig (relatedConfig.param)}
    {#if UI_WANTED_FILTERS[String(endpoint.current)]?.includes(relatedConfig.param)}
      ...
      >
        <h3 class="mb-0 text-sm font-bold tracking-wide text-black">
          {relatedConfig.sectionTitle}
        </h3>
        <div
          class="flex w-full flex-row items-center justify-between"
          style="min-height: auto; padding-top: 0.25rem; padding-bottom: 0.125rem; margin-bottom: 0.25rem;"
        ...
        {#if paramConfig}
          ...
          >
            ...
            <div class="w-full">
              <SelectDropdown
                ...
                disabled={paramConfig.disabledExtra() ||
                  (!filters.isRelatedMode && filters.getOptions(paramConfig.param).length === 0)}
            ...
            <meta itemprop="value" content={String(filters.getParam(paramConfig.param) || '')} />
          </div>
        {/if}
      </div>
    {/if}
  ...
  {#if hasActiveFilters}
    <div
      id="active-filters"
      class="flex flex-col gap-0 rounded-2xl border-2 border-blue-400 bg-linear-to-br from-blue-200/90 via-blue-100/90 to-blue-50/80 px-2 py-0.5 text-xs font-normal text-black shadow-lg"
      style="width:100%;box-sizing:border-box;padding-top:0.25rem;"
    >
      <div class="mb-0 flex items-center gap-1">
        <span class="text-sm font-normal tracking-wide text-black">Active Filters</span>
        <svg
      ...
      <ul class="mb-0 list-disc pl-4 text-xs leading-tight">
        {#each activeFilterItems.filter((item) => item.show) as item (item.key)}
          ...
          >
            ...
            {#if item.label}
              <span class="font-normal text-black" itemprop="name">{item.label}:</span>
            {/if}
            <span class="font-normal text-black" itemprop="value">{item.value || item.label}</span>
          </li>
        {/each}
      </ul>
