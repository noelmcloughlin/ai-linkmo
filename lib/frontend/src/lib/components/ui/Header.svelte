<script lang="ts">
  ...
  // Props (runes)
  let { isRelatedMode = false, isRecordViewer = false, recordViewerTitle = '' } = $props();
  ...
  <div class="header-content bg-linear-to-r from-[#43b02a] via-blue-100 to-blue-400 shadow-md">
    ...
    <div class="header-center">
      <span class="header-center-text flex items-center gap-2">
        {#if isRecordViewer}
          <span class="font-bold text-green-900">[{endpoint.getLabel().toUpperCase()}]:</span>
        ...
        {:else}
          ...
          <span class="header-count">
            {#key `${endpoint.isLoading ? 'isLoading' : filtered.items.length}`}
              {#if endpoint.isLoading}
                ...
                >
                  ...
                  <path
                    class="opacity-75"
                    fill="currentColor"
                    d="M4 12a8 8 0 018-8V0a4 4 0 00-4 4H4z"
                  ></path>
                </svg>
              ...
              {:else}
                <span class="header-count-badge"> (0) </span>
              {/if}
            {/key}
          </span>
        {/if}
      ...
      <div class="flex w-full items-center justify-between gap-3">
        {#if !isRecordViewer}
          ...
          <button
            ...
            class="flex items-center gap-2 rounded-lg px-3 py-2 transition-all duration-200 {zenMode
          ...
          {#if !authState.loggedIn}
            <Button variant="login" size="lg" onClick={() => login()}>Login</Button>
