...
def get_ai_atlas_nexus_paths():
    # Try to find the installed location of ai_atlas_nexus
    spec = importlib.util.find_spec("ai_atlas_nexus")
    if not spec or not spec.submodule_search_locations:
        raise ImportError("ai_atlas_nexus package not found in current environment.")
    base_path = Path(list(spec.submodule_search_locations)[0])
    mapping_dir = base_path.parent / "data" / "mappings"
    schema_dir = base_path / "ai_risk_ontology" / "schema"
    return str(mapping_dir) + "/", str(schema_dir) + "/"

MAPPING_DIR, SCHEMA_DIR = get_ai_atlas_nexus_paths()
OUTPUT_DIR = "graph/cypher/"
...
def is_relationship(
    ...
    slot_names = [slot.name for slot in class_slots]
    if linkml_slot not in slot_names:
        # Skip unknown slots instead of raising an error
        return False
