...
2. **The bundle-root `index.md` carries the semantic header.** It declares the keys that lift the whole bundle into RDF (values shown are illustrative - the real ones are minted at scaffolding time):
   ...
   **Choosing `base_iri` - identifiers, not hyperlinks, but pick a namespace you control.** A concept's `id` is a globally unique *name*, like a namespaced UUID that looks like a URL - nothing requires that pasting the (url-like) concept `id` into a browser return a page, so a 404 on a concept `id` is *valid*. It is still a design smell worth flagging: Linked Data best practice ("Cool URIs") is that these identifiers *should* eventually double as working links. The test for a good `base_iri` is **authority + future resolvability**:
   ...
   - **Migrate early if the base is wrong.** Changing `base_iri` rewrites every concept `id` and breaks external links to the old identifiers if they exist - cheap while the bundle is young, increasingly expensive after. When migrating: replace the namespace in `base_iri`, the publisher `id`, every concept `id`, and all typed-relation targets; leave `resource`/`distribution` URLs alone (those are real links, not minted identifiers); log the migration and its rationale in `log.md`; re-run `just lokf-validate` and confirm the converted graph output contains only the new namespace.
   - **If a human asks "these IDs don't resolve - is that a problem?"** the answer is: valid by design, but check the two-part test above; if the namespace is uncontrolled it can never resolve and should be migrated, and if it is controlled but not yet registered (at https://w3id.org for example), note the pending redirect registration rather than treating 404 as a defect.
3. **Use a class from the LOKF type vocabulary** (consumers tolerate unknowns as `lokf:Concept`): `Dataset`, `Table`, `Metric`, `Service`, `Playbook`, `Policy`, `GlossaryTerm`, `Reference`, `Document`, `Person`, `Organization`. Type-specific fields: `Table/Dataset` -> `fields`, `distribution`; `Metric` -> `unit`, `formula`, `measures`; `Service` -> `endpoint`, `http_method`, `documentation`; `GlossaryTerm` -> `definition`, `abbreviation`. Structured-value fields — `distribution` items are `Distribution` objects (`access_url`, `name?`, `description?`, `media_type?`); `fields` items are `Field` objects (`name?`, `description?`, `datatype?`, `is_key?`, `unit?`, `constraints?`). Do NOT use plain URL strings or plain strings for these.
